...
const basicAuthMiddleware = (req, res, next) => {
    ...
    if (username !== WEBHOOK_USERNAME || password !== WEBHOOK_PASSWORD_LAVA) {
...
app.post('/crypto_payment', async (req, res) => {
    try {
        ...
        if (Status === 'Success') {
            if (order.output.id === PaymentId && !Test) {
                ...
                if (TotalAmountUSD >= minAcceptableAmount) {
                    if (!order.isGift) {
                        if (!order.isRenew) {
                            console.log(
                                'Payment processed successfully and amount verified with slippage tolerance.',
                            );

                            const isSuccess =
                                await orderService.setOrderSuccessAndGiveAccess(
                                    BillingID,
                                );
                            ...
                            if (isSuccess) {
                                ...
                                console.log(
                                    'Payment successful but something wrong',
                                );
                            ...
                            const isSuccess =
                                await orderService.renewSubscription(
                                    order.orderId,
                                );
                            if (isSuccess) {
                                await bot.telegram.sendMessage(
                                ...
                                console.error(
                                    'Failed to update order status after successful payment.',
                                );
                                await bot.telegram.sendMessage(
                                    userId,
                                    ERROR_UNDEFINED_PAYMENT,
                        ...
                        const newPromo =
                            await new PromoService().createPromoCode(
                                userId,
                                tariff,
                            );
                        ...
                        await bot.telegram.sendMessage(
                            userId,
                            PAYMENT_SUCCESSFUL_GIFT(
                                `https://t.me/${BOT_NAME}?promo=${newPromo.promoCode}`,
                            ),
                            {
                                parse_mode: 'HTML',
                                disable_web_page_preview: true,
                    ...
                    console.error(
                        `Received amount ${TotalAmountUSD} is less than acceptable amount ${minAcceptableAmount}.`,
                    );
                    await bot.telegram.sendMessage(
                        userId,
                        ERROR_INSUFFICIENT_AMOUNT,
                ...
                console.log(
                    'PaymentID DONT MATCH WITH DATA IN DATABASE OR TEST IS SET',
                );
            }
            ...
            await bot.telegram.sendMessage(
                userId,
                ERROR_PAYMENT_CANCELLED(order.output.address, tariff),
            );
    ...
    try {
        ...
        if (data.parentContractId !== undefined) {
            const originalOrderId =
                await paymentFiatService.getOrderIdByParent();
        ...
        try {
            if (status === 'subscription-active' || status === 'completed') {
                ...
                await paymentFiatService.saveParentIdToOrder(
                    contractId,
                    order.orderId,
                );
                ...
                if (!order.isGift) {
                    if (!order.isRenew) {
                        ...
                        const isSuccess = await orderService.renewSubscription(
                            order.orderId,
                        );
                        if (isSuccess) {
                            await bot.telegram.sendMessage(
                    ...
                    const newPromo = await new PromoService().createPromoCode(
                        userId,
                        tariff,
                    );
