...
export const payCardPackagesScreen = async (ctx, editMessage) => {
  ...
  const keyboard = [
    [ { text: `🤝 ${t.start.title} (${t.start.rub}₽)`, command: 'order_card_start' } ],
    [ { text: `😎 ${t.pro.title} (${t.pro.rub}₽)`, command: 'order_card_pro' } ],
    [ { text: `👑 ${t.premium.title} (${t.premium.rub}₽)`, command: 'order_card_premium' } ],
